Function Invoke-PSMsSPOBatchRequest {
    <#
    .SYNOPSIS
    ...
    .DESCRIPTION
        ...
        For each batch payload, this function can validate the requests (Test-PSMicrosoftSharepointOnlineBatchRequest),
        ...
        This allows a subsequent cmdlet (e.g. Invoke-PSMicrosoftSharepointOnlineBatchResponse) to correlate them by id.
    ...
    .EXAMPLE
        PS C:\> payloads = New-PSMsSPOBatchRequest -InputObject $requests
        PS C:\> $result = $payloads | Invoke-PSMsSPOBatchRequest -EnableException -WhatIf -Force

        Suppose $payloads are returned from New-PSEntraIDBatchRequest -InputObject $requests

        PS C:\>  ready to be analyzed by a separate cmdlet that correlates them.
