function ConvertFrom-RestSite {
	...
	.EXAMPLE
		PS C:\> Invoke-RestRequest -Service 'graph' -Path users -Query $query -Method Get -ErrorAction Stop | ConvertFrom-RestSite
	...
	$type = if ($InputObject -is [array]) {
		[PSMicrosoftSharepointOnline.Sites.Site[]]
	}
	else {
		[PSMicrosoftSharepointOnline.Sites.Site]
	}
